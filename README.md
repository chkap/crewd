...
The role consumes its inbox file at the start of its next tick and truncates it. `OVERRIDE` outranks the role's own plan; `ADVICE` is treated as a strong suggestion; `INFO` is context only.
